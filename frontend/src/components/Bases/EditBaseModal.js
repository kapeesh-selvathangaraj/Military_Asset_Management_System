...
const EditBaseModal = ({ isOpen, onClose, base }) => {
  ...
  // Initialize form data when base prop changes
  useEffect(() => {
    if (base) {
      setFormData({
        name: base.name || '',
        ...
        contact_info: {
          phone: base.contact_info?.phone || '',
          email: base.contact_info?.email || '',
          address: base.contact_info?.address || ''
        }
      });
    }
  }, [base]);
  ...
  const updateBaseMutation = useMutation(
    async (baseData) => {
      const response = await api.put(`/api/bases/${base.id}`, baseData);
      return response.data;
    },
    {
      onSuccess: () => {
      ...
      onError: (error) => {
        toast.error(error.response?.data?.message || 'Failed to update base');
      }
    }
  );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updateBaseMutation.mutate(formData);
  };
